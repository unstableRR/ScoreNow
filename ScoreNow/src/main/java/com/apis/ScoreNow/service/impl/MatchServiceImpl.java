package com.apis.ScoreNow.service.impl;

import com.apis.ScoreNow.entities.Match;
import com.apis.ScoreNow.repositories.MatchRepo;
...
//                update the match in database
    ...
    @Override
    public List<List<String>> getpointTableWC() {
...
//                    System.out.println(points);
    ...
    @Override
    public List<List<List<String>>> getpointTableRanjhi() {
        System.out.println("Entering");
        List<List<List<String>>> allPointTables = new ArrayList<>();
        String tableUrl = "https://www.cricbuzz.com/cricket-series/6725/ranji-trophy-2023-24/points-table";
        try{
            Document doc = Jsoup.connect(tableUrl).get();
            Elements tables = doc.select("table.cb-srs-pnts");
            for(Element table : tables){

                List<List<String>> groupTables = new ArrayList<>();
                Elements tableHeads = table.select("thead>tr>*");
                List<String> headers = new ArrayList<>();
                tableHeads.forEach(element -> {
                    headers.add(element.text());
                });
                groupTables.add(headers);
                Elements bodyTrs = table.select("tbody>*");
                bodyTrs.forEach(tr -> {
                    List<String> points = new ArrayList<>();
                    if (tr.hasAttr("class")) {
...
//                    System.out.println(points);
                        groupTables.add(points);
                    }


                });

                //System.out.println(groupTables);
                allPointTables.add(groupTables);
        ...
        System.out.println(allPointTables);

        return allPointTables;
    }
}
